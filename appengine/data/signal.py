...
class SignalEngine(object):
    ...
    for checkin in json_checkins:
      ...
      if source == 'facebook':
        ...
        if constants.FOURSQUARE_FOOD_PARENT in [i.lower() for i in checkin['data']['categories']['parents']]:
          parsed_signal = self.ParseFoursquareJSON()
      if parsed_signal:
  ...
  def CheckForDuplicateCheckin(self, parsed_signal, place):
    ...
    duplicate = duplicate.filter(data_models.Checkin.who == users.get_current_user.email())
  ...
  def AddCheckinToPlace(self, parsed_signal, place):
    ...
    new_checkin.who = users.get_current_user().email()
  ...
  def ParseFacebookJSON(self, signal):
    ...
    parsed_signal['city'] = signal['data']['location']['city']
    parsed_signal['state'] = signal['data']['location']['state']
    parsed_signal['type'] = self._FoursquareDetermineType(signal)
    parsed_signal['friend'] = '%s %s' % (signal['data']['user']['firstName'], signal['data']['user']['lastName'])
    parsed_signal['mentions'] = signal['data']['venue']['stats']
    parsed_signal['when'] = signal['data']['createdAt']
    parsed_signal['data'] = signal['data']['user']['trips']
